...
def aplicar_estilo_blindado():
    ...
    /* Sidebar Fundo Azul e Textos Brancos */
    ...
    /* Letras Pretas no Upload e Inputs */
    ...
    /* Corpo do App (Letras Escuras e Títulos Tecadi) */
    .stApp { color: #1E1E1E; }
...
def processar_motor(arquivo_novo, data_selecionada):
    ...
    # "SEGURANÇA NA PORTA": Só passa o que é transitório de verdade
    ...
    if df_hist.empty:
        ...
        # Se mudou de endereço transitório, soma +1 movimento e anota de onde veio
        ...
        # 2. NOVOS SERIAIS (Entraram hoje na área transitória)
        ...
        # 3. SAÍDAS (Foram pro estoque definitivo)
...
with st.sidebar:
    ...
    st.markdown("### 🔄 Motor de Atualização")
    data_batimento = st.date_input("Data do Arquivo:", value=datetime.today(), format="DD/MM/YYYY")
    arquivo = st.file_uploader("Anexe o Batimento (.xlsx)", type=['xlsx'])
    
    if arquivo and st.button("Sincronizar Retenção", use_container_width=True, type="primary"):
        with st.spinner("Mapeando histórico de seriais..."):
...
if df_full.empty:
    st.info("A base está vazia. Informe a data correta e anexe o primeiro batimento para criar o marco zero.")
    st.stop()
...
if not df_pendente.empty:
    # --- GRÁFICO DO PROBLEMA (SÓ 1+ DIAS) ---
    st.subheader("🔥 Foco de Atuação: Seriais Retidos há 1 Dia ou Mais")
    st.markdown("Este gráfico isola o problema. Mostra apenas o volume de produtos que já estouraram o SLA de transição.")
    ...
    if not df_problema.empty:
        ...
        grafico_problema = grafico_problema.sort_values('Qtd_Retida', ascending=False)
        
        fig = px.bar(
            grafico_problema, x="AZ", y="Qtd_Retida", color="RESPONSAVEL",
            text_auto=True, template="plotly_white",
            labels={"AZ": "Área Transitória (AZ)", "Qtd_Retida": "Qtd. de Seriais Vencidos"},
            color_discrete_sequence=px.colors.qualitative.Bold
        )
        fig.update_layout(xaxis={'categoryorder':'total descending'}, legend_title_text='Dono da Área')
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.success("🎉 Excelente! Não há nenhum serial retido por mais de 1 dia nas áreas selecionadas.")

    st.markdown("---")

    # --- TABELA INVESTIGATIVA (A FICHA DO SERIAL) ---
    st.subheader("📋 Mapa de Rastreio Operacional")
    st.markdown("Visão completa para a equipe localizar o serial e entender a sua rota dentro da área transitória.")
    
    # Formatação das colunas para visão executiva
    ...
    df_detalhe.rename(columns={
        ...
        'DT Entrada': 'Data de Chegada'
    }, inplace=True)
    
    # Ordena para os problemas piores ficarem sempre no topo (Mais dias parados e com mais movimentos inúteis)
    df_detalhe = df_detalhe.sort_values(by=["Dias Pendentes", "Mov. Internas", "Prioridade"], ascending=[False, False, True])
    
    # Formata a data de chegada para aparecer bonitinha na tabela sem as horas
    df_detalhe['Data de Chegada'] = df_detalhe['Data de Chegada'].dt.strftime('%d/%m/%Y')
    
    st.dataframe(df_detalhe, use_container_width=True, hide_index=True, height=450)
    ...
    st.download_button(
        label="📥 Extrair Relatório Completo para Excel",
